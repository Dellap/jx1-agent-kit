...
|---|---|---|
| Vận hành/khởi động/sửa lỗi server JX1 (WSL2) | `skills/<SSH_ALIAS>-server-ops/SKILL.md` | 5 service, thứ tự start, FixIp, log chẩn đoán |
...
| Sửa skill/giá trị theo cấp (Lua `.lua`, `skills.txt`, `Missile.txt`) | `skills/<SSH_ALIAS>-skill-data-modding/SKILL.md` + `references/*.md` | 3 lớp phải khớp: skills.txt ↔ .lua ↔ Missile.txt |
| Mod UI client (.ini, .spr, pak, GBK filename, theme, resolution, PartType) | `skills/<SSH_ALIAS>-client-modding/SKILL.md` (+ `references/*`) | Engine chỉ render control đã khai báo. Skill `jx1-client-ui-modding` đã **gộp vào đây** (17/09/2026) |
...
| **Cài 1 mod client** từ `Update/<tên mod>/` (đọc README của mod rồi copy/ghi đè) + chẩn đoán "bấm không thấy gì" | `skills/<SSH_ALIAS>-client-modding/references/mod-install-and-debug.md` | Kiểm cú pháp Lua trước khi đẩy; nhiều file trong `2_server/` của mod chỉ là **ghi chú**, đè là mất file thật |
| Bảng nhiệm vụ TaskTrace / cửa sổ `battle_select` (ca 17/09) | `skills/<SSH_ALIAS>-client-modding/references/{jx1-battleselect-taskextrace.md, tasktrace-panel-layout.md}` | `battle_select.ini` là cửa sổ **NEO dùng chung** — KHÔNG đè ini gốc |
...
- ❌ Không đặt **tên Unicode** cho file/thư mục tiếng Trung khi copy vào client: mọi tên trong client là **mojibake**
  (bytes GBK đọc như Latin-1) — đặt sai tên là engine không tìm thấy (xem `<SSH_ALIAS>-client-modding`).
...
```
skills/<SSH_ALIAS>-server-ops/SKILL.md   # server chạy thế nào, restart ra sao
skills/CHEATSHEET.md              # path, port, log, lệnh hay dùng (trang facts)
```

## Quy ước placeholder (đường dẫn mỗi máy mỗi khác)

Tài liệu trong repo **không giả định ổ đĩa/thư mục của một máy cụ thể**. Khi gặp:

| Placeholder | Nghĩa | Cách tự xác định |
|---|---|---|
| `<GAME_ROOT>` | thư mục chứa `Client/` (có `vdk.dll`, `game.exe`, `JX1Mod.ini`) | tìm thư mục có `Client/vdk.dll` |
| `<WORK_DIR>` | thư mục cha của game root (`Update/`, `Tools/`, `_unpack/`) | thư mục cha của `<GAME_ROOT>` |
| `<UNPACK_DIR>` | nơi gom output bóc pak | tự chọn, đừng rải trong `<GAME_ROOT>` |
| `<PORTABLE_DIR>` | thư mục script khởi động server của bạn | nơi có `boot_all.sh`/launcher |
| `<SSH_ALIAS>` | alias SSH tới máy chạy game | mục `Host` trong `~/.ssh/config` |
| `/home/jxser` | **chuẩn của gói server**, không phải lựa chọn riêng | gói VLTK HKMP mặc định `/home/jxser` |

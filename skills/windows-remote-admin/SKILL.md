...
- **Elevation is PER SSH path, never assume**: `net session >nul && echo ELEVATED`. Direct Windows OpenSSH as `<SMB_USER>` was ELEVATED (netsh/schtasks/firewall worked). But `ssh jx1` (root@<GAME_HOST_IP>:2222, WSL-side sshd) runs as Windows user <WIN_USER> at **medium IL** — `whoami /groups` shows Administrators "Group used for deny only" → netsh add, `schtasks /rl highest`, `attrib -r` all return Access denied.
